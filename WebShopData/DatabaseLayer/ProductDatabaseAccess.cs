...
namespace WebShopData.DatabaseLayer
{
    public class ProductDatabaseAccess : IProductAccess
    {
         ...
         public List<Product> GetProductAll()
        {
            ...
            Product readProduct;
            string queryString = "select id, name, price, image, stockQuantity, isStock from Product";
            using (SqlConnection con = new SqlConnection(_connectionString))
        ...
        private Product GetProductFromReader(SqlDataReader productReader)
        {
            ...
            foundProduct = new Product(tempId, tempName, tempPrice, tempImage, tempStockQuantity, tempIsStock );
        ...
        {
            Product foundProduct;
            string queryString = "select ID, name, price, image, stockQuantity, isStock from Product where id = @Id";
            using (SqlConnection con = new SqlConnection(_connectionString))
            using (SqlCommand readCommand = new SqlCommand(queryString, con))
            {
                // Prepace SQL
